...
export default async function SavedBillionaireProfilePage({
  params,
}: {
  params: { id: string };
}) {
  ...
  if (!profile || profile.type !== "billionaire") {
    notFound();
  }
  ...
  for (const hint of parseJsonArray(profile.entity.source_hints)) {
    const match = hint.match(/^dataset:(\w+)=(.+)$/);
    if (match) {
      csvFields[match[1]] = match[2];
    }
  }
    ...
    <main className="min-h-screen bg-black text-white">
      <div className="mx-auto max-w-4xl px-6 py-8">
        <div className="flex items-center justify-between gap-4">
          <Link href="/billionaires/saved" className="text-sm opacity-80 hover:opacity-100">
            Back
          </Link>
          ...
          <div className="mt-8 space-y-6">
            {educationSection && (
              <section className="rounded-xl border border-white/10 bg-black/30 p-5">
                <h2 className="text-lg font-semibold tracking-tight">{educationSection.heading}</h2>
                <p className="mt-3 text-sm leading-relaxed opacity-80">{educationSection.body}</p>
              </section>
            )}

            {startedSection && (
              <section className="rounded-xl border border-white/10 bg-black/30 p-5">
                <h2 className="text-lg font-semibold tracking-tight">{startedSection.heading}</h2>
                <p className="mt-3 text-sm leading-relaxed opacity-80">{startedSection.body}</p>
              </section>
            )}
