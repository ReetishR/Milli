...
export default function CompaniesPage() {
  ...
  const query = useMemo(() => {
    const base = "/api/serve?type=company&mode=new";
    return activeTag ? `${base}&tag=${encodeURIComponent(activeTag)}` : base;
  }, [activeTag]);
          ...
          {FILTERS.map((f) => {
            ...
            return (
              <button
                ...
                className={
                  active
                    ? "rounded-full bg-white text-black px-3 py-1 text-xs"
                    : "rounded-full bg-white/10 px-3 py-1 text-xs hover:bg-white/15"
                }
              >
                {f.label}
              </button>
